...
public class GUIImpl extends JFrame implements GUIInterface {
    ...
    public void buildMenus() {
        ...
        file.add(item = new JMenuItem("Stream ROM..."));
        item.addActionListener(listener);
    ...
    /**
     * Método que inicia o Stream de ROMs com a primeira iteração, que é receber
     * a lista de jogos e pedir para o usuário selecionar um deles.
     */
    public void streamRom() {
        try {
            ...
            dos.writeInt(1); // Entra na primeira iteração
            
            String listaJogos = dis.readUTF(); // É recebida a lista de jogos do servidor
            
            // Abre-se uma janela requisitando ao usuário que selecione um jogo
            GamesListStream lista = new GamesListStream(this, listaJogos);
            lista.setVisible(true);
            
            s.close(); // Fecha-se a conexão
        } catch(Exception e) {
            messageBox("Conexão falhou!");
            e.printStackTrace();
        }
    }
    
    /**
     * Segunda parte da Stream de ROM, onde recebe-se o nome da ROM selecionada
     * para ser baixada, conecta-se novamente ao servidor e executa a segunda
     * iteração. Nela, é passado ao servidor o nome da ROM a ser baixada, e inicia-se
     * a transferência do arquivo do servidor ao cliente (usuário).
     * @param nomeRom String contendo o nome da ROM selecionada no formulário "GamesListStream"
     */
    public void streamRom(String nomeRom) {
        try {
            String host = "localhost";
            int port = 1234;
            Socket s = new Socket(host, port);
            ...
            dos.writeInt(2); // Entra na segunda iteração
            
            dos.writeUTF(nomeRom); // Se passa o nome da ROM a ser recebida
            
            ObjectInputStream ois = new ObjectInputStream(s.getInputStream());
            receberRom(ois, nomeRom); // Inicia-se o recebimento da ROM
            
            s.close(); // Fecha-se a conexão
            
            loadROM("C:\\TEMP\\" + nomeRom + ".nes"); // É carregada a ROM
        } catch(Exception e) {
            messageBox("Conexão falhou!");
            e.printStackTrace();
        }
    ...
    /**
     * Método de recebimento da ROM.
     * @param ois Stream de entrada.
     * @param nomeRom String contendo o nome da ROM.
     */
    public void receberRom(ObjectInputStream ois, String nomeRom) {
        try {
            // É criado a saída de arquivo, passando como parâmetro a pasta temporária em que será armazenada a ROM
            FileOutputStream fos = new FileOutputStream("C:\\TEMP\\" + nomeRom + ".nes");
            byte[] buffer = new byte[200000];
            
            // Recebendo a ROM
            while(true) {  
                int len = ois.read(buffer); // É lido o buffer enviado pelo servidor
                if(len == -1) {
                    break;
                }  
                fos.write(buffer, 0, len); // É escrito o buffer lido na saída do arquivo
            }  
            
            fos.flush();  
            fos.close(); // Fecha-se o Stream de saída
        } catch(Exception e) {}
    }
    ...
    public class AL implements ActionListener, WindowListener {
        ...
        public void actionPerformed(final ActionEvent arg0) {
            ...
            } else if (arg0.getActionCommand().equals("Stream ROM...")) { // Botão de menu implementado neste trabalho
                messageBox("Conectando com o servidor...");
                streamRom(); // Inicia o Stream de ROM
            }
        }
